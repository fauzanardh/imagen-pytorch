...
class ElucidatedImagen(nn.Module):
    ...
    def preconditioned_network_with_cond_scale(
        ...
        **kwargs,
        ...
        logits = self.preconditioned_network_forward(
            *args, cond_drop_prob=cond_drop_prob, **kwargs
        )

        if cond_scale in [1.0, 0.0]:
            return logits

        null_logits = self.preconditioned_network_forward(
            *args, cond_drop_prob=1.0, **kwargs
        )
    ...
    def forward(
        ...
        if exists(random_crop_size):
            ...
            if is_video:
                images, lowres_cond_img = rearrange_many(
                    (images, lowres_cond_img), "b c f h w -> (b f) c h w"
                )
            ...
            if is_video:
                images, lowres_cond_img = rearrange_many(
                    (images, lowres_cond_img), "(b f) c h w -> b c f h w", f=frames
                )
