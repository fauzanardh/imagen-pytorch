...
from imagen_pytorch.t5_encoder import get_encoded_dim, DEFAULT_T5_NAME
...
class Unet3D(nn.Module):
    def __init__(
        ...
        *,
        ...
        def temporal_peg(dim):
            return Residual(
                nn.Sequential(
                    Pad(temporal_peg_padding),
        ...
        def temporal_attn(dim):
            EinopsToAndFrom(
                "b c f h w",
                "(b h w) f c",
                Residual(Attention(dim, **{**attn_kwargs, "causal": time_causal_attn})),
            )
