...
@click.option(
    "--cond_scale",
    default=5,
    help="conditioning scale (classifier free guidance) in decoder",
)
@click.option("--load_ema", default=True, help="load EMA version of unets if available")
@click.argument("text")
def sample(model, cond_scale, load_ema, text):
    ...
    print(
        f"loading Imagen from {full_model_path}, saved at version {version} - current package version is {__version__}"
    )

    # get imagen parameters and type

    imagen = load_imagen_from_checkpoint(
        str(model_path), load_ema_if_available=load_ema
    )
    imagen.cuda()
...
@click.option(
    "--path", default="./imagen_config.json", help="Path to the Imagen model config"
)
def config(path):
...
@click.option(
    "--config", default="./imagen_config.json", help="Path to the Imagen model config"
)
@click.option(
    "--unet",
    default=1,
    help="Unet to train",
    type=click.IntRange(1, 3, False, True, True),
)
@click.option("--epoches", default=1000, help="Amount of epoches to train for")
@click.option(
    "--text", required=False, help="Text to sample with between epoches", type=str
)
@click.option(
    "--valid",
    is_flag=False,
    flag_value=50,
    default=0,
    help="Do validation between epoches",
    show_default=True,
)
def train(config, unet, epoches, text, valid):
    # check config path
    ...
    imagen_config_klass = (
        ElucidatedImagenConfig if config_data["type"] == "elucidated" else ImagenConfig
    )
    imagen = imagen_config_klass(**config_data["imagen"]).create()

    trainer = ImagenTrainer(imagen=imagen, **config_data["trainer"])
    ...
    max_batch_size = (
        config_data["max_batch_size"] if "max_batch_size" in config_data else 1
    )

    channels = "RGB"
    if "channels" in config_data["imagen"]:
        assert (
            config_data["imagen"]["channels"] > 0
            and config_data["imagen"]["channels"] < 5
        ), "Imagen only support 1 to 4 channels L, LA, RGB, RGBA"
    ...
    assert (
        "batch_size" in config_data["dataset"]
    ), "A batch_size is required in the config file"
    ...
    for i in range(epoches):
        ...
        if valid != 0 and not (i % valid) and i > 0:
            valid_loss = trainer.valid_step(
                unet_number=unet, max_batch_size=max_batch_size
            )
